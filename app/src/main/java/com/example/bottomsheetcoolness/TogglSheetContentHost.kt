...
internal fun TogglSheetContentHost(
    ...
    if (backStackEntry != null) {
        ...
        backStackEntry.LocalOwnersProvider(saveableStateHolder) {
            Box(Modifier
                .onGloballyPositioned { contentPositionedSignal.complete(Unit) }
                .imePadding()
            ) {
...
private suspend fun TogglModalBottomSheetState.internalHide() {
    snapTo(TogglModalBottomSheetValue.Hidden)
}

@OptIn(ExperimentalMaterialApi::class)
private val TogglModalBottomSheetState.willBeVisible: Boolean
    get() = targetValue == TogglModalBottomSheetValue.HalfExpanded || targetValue == TogglModalBottomSheetValue.Expanded
